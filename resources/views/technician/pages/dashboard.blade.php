                    ...
                    <div class="card-body px-0 pt-0 pb-2">
                        <div class="table-responsive p-0">
                            <table class="table align-items-center mb-0" id = 'myTable'
                                style="background-color:rgb(90, 88, 245)" id = 'myTable'>
                                <thead>
                                    <tr>
                                        <th>id</th>
                                        <th>name</th>
                                ...
                                <tbody>
                                    @foreach ($order as $order)
                                        <tr>
                                            ...
                                            <td>
                                                <span
                                                    class="badge bg-@if ($order->status == 'accepted') success @endif
                                            ">
                                                    {{ $order->status }}
                                                </span>
                                            </td>
                                            <td>
                                                <form action="{{ route('technicianpanel.store', $order->id) }}"
                                                    method="post">
                                                    @csrf
                                                    <div class="d-flex mt-2">
                                                        <button type="submit" class="btn btn-secondary">Accept
                                                            Order</button>
                                                    </div>
                                                </form>
                        </div>
                        ...
                        <td>
                            <div class="d-flex mt-2" style="gap: 5px">
                                <a href="{{ route('technicianpanel.pages.view', $order->id) }}"
